<template>
  <div class="app-container h-screen overflow-hidden bg-[var(--color-background)] text-[var(--color-text-primary)] flex flex-col lg:flex-row font-sans relative select-none">
    ...
    <aside 
      ...
      role="navigation"
      aria-label="Navigation principale distributeur"
    >
      <!-- BRAND IDENTITY -->
      <div class="h-16 border-b border-[var(--color-border)] px-4 flex items-center justify-between shrink-0 select-none bg-[var(--color-surface-elevated)]">
        <div class="flex items-center space-x-2.5 truncate" v-show="!isCollapsed || isMobileMenuOpen">
          ...
          <div class="truncate">
            ...
            <span class="text-xs text-[var(--color-text-secondary)] block truncate">Espace Distributeur</span>
          </div>
        </div>
      ...
      >
        <div class="h-10 w-10 bg-[var(--color-primary-muted)] text-[var(--color-primary)] border border-[var(--color-primary-border)] flex items-center justify-center font-bold text-sm rounded-full shadow">
          {{ authStore.user?.name ? authStore.user.name[0].toUpperCase() : 'D' }}
        </div>
        <!-- Tooltip -->
        ...
        <div v-for="group in filteredMenu" :key="group.label" class="space-y-2">
          ...
          <div v-for="item in group.items" :key="item.name" class="space-y-1">
            ...
            >
              ...
              >
                <component :is="item.icon" class="h-5 w-5 mr-3 shrink-0 stroke-[2.5px]" :class="getIconGroupColor(group.label, isRouteActive(item.to))" aria-hidden="true" />
    ...
    <div class="page-container flex-1 flex flex-col min-w-0 overflow-y-auto max-h-screen bg-[var(--color-background)]">
      ...
      <header class="h-16 border-b border-[var(--color-border)] bg-[var(--color-surface)]/90 backdrop-blur px-6 flex items-center justify-between shrink-0 sticky top-0 z-10 select-none shadow-sm">
        ...
        <div class="flex items-center space-x-2 text-sm text-[var(--color-text-secondary)] font-medium truncate">
          <router-link to="/distributor/dashboard" class="hover:text-[var(--color-primary)] transition">Distributeur</router-link>
        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="flex items-center space-x-3">
            <div class="hidden sm:flex flex-col items-end">
              <span class="text-sm font-semibold text-[var(--color-text-primary)] leading-none">{{ authStore.user?.name || 'Distributeur' }}</span>
...
const rawMenuStructure = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        children: [
          { name: 'Tous les entrepôts', to: '/distributor/warehouses', icon: ListBulletIcon },
          { name: 'Ajouter un entrepôt', to: '/distributor/warehouses/create', icon: PlusIcon },
          { name: 'Transferts de stock', to: '/distributor/warehouses/transfers', icon: ArrowPathIcon },
          { name: 'Ajustements', to: '/distributor/warehouses/adjustments', icon: AdjustmentsHorizontalIcon }
        ]
      },
      {
        id: 'products',
      ...
      },
      {
        ...
        icon: InboxIcon,
        children: [
          { name: 'Devis reçus', to: '/distributor/rfq/received', icon: ArrowDownTrayIcon },
          { name: 'Devis émis', to: '/distributor/rfq/sent', icon: ArrowUpTrayIcon }
        ]
      }
    ]
  },
  {
    label: 'Approvisionnement Amont',
    items: [
      {
        id: 'suppliers',
        name: 'Fournisseurs',
        icon: BuildingStorefrontIcon,
        children: [
          { name: 'Liste des fournisseurs', to: '/distributor/suppliers', icon: ListBulletIcon },
          { name: 'Ajouter un fournisseur', to: '/distributor/suppliers/create', icon: UserPlusIcon }
        ]
      },
      {
        id: 'purchases',
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        name: 'Paramètres',
        icon: Cog6ToothIcon,
        to: '/distributor/settings/general'
      }
    ]
  }
];
...
const breadcrumbs = computed(() => {
  ...
  // Skip the first segment (e.g. 'distributor')
  ...
  return relevantSegments.map((seg, index) => {
    ...
    if (label.toLowerCase() === 'warehouses') label = 'Entrepôts';
    if (label.toLowerCase() === 'clients') label = 'Clients';
    if (label.toLowerCase() === 'settings') label = 'Paramètres';
    if (label.toLowerCase() === 'general') label = 'Général';
    if (label.toLowerCase() === 'received') label = 'Reçus';
    if (label.toLowerCase() === 'sent') label = 'Émis';
