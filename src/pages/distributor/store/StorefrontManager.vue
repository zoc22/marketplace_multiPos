  ...
  <div class="space-y-6">
    ...
    <div v-if="activeTab === 'catalog'" class="bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-2xl overflow-hidden shadow-sm">
      <div class="p-6 border-b border-[var(--color-border)] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h3 class="font-bold text-base text-[var(--color-text-primary)]">Publications des Produits B2B</h3>
...
const availableWidgets = [
  { name: 'Bannière Héro Promos', type: 'hero', title: 'GRANDES PROMO B2B', subtitle: 'Achat de Céréales & Fèves en Gros', description: 'Profitez de nos remises exceptionnelles sur volumes.' },
  { name: 'Grille des Produits', type: 'products', title: 'Notre Sélection en Gros' },
  { name: 'Bloc d\'Informations', type: 'info', title: 'Livraison Logistique Réseau', content: 'Fret acheminé par nos soins à travers tout le territoire national.' },
  { name: 'Formulaire de Contact / Devis', type: 'contact' },
  { name: 'Pied de page (Réassurance B2B)', type: 'footer', features: [{ title: 'Fret Assuré', desc: '100% garanti' }, { title: 'Paiement Termes', desc: 'Jusqu\'à 30 jours' }, { title: 'SAV Logistique', desc: 'Retour ou remplacement' }] }
];
...
const initBuilder = () => {
  ...
  if (saved) {
    ...
    builderConfig.value = {
      ...
      animation: 'none',
      widgets: [
        { id: 'w_hero', type: 'hero', title: 'GRANDES PROMO B2B', subtitle: 'Achat de Céréales & Fèves en Gros', description: 'Profitez de nos remises exceptionnelles sur volumes.' },
        { id: 'w_prod', type: 'products', title: 'Notre Sélection en Gros' }
      ]
    };
  }
};
...
const generatePublicLink = () => {
  sessionStorage.setItem('products_distributor', JSON.stringify(publishedProducts.value));
  saveBuilderDesign();
...
const mockWarehouses = [
  { id: 'wh_douala_1', name: 'Entrepôt Logistique Douala I' },
  { id: 'wh_yaounde_1', name: 'Entrepôt Logistique Yaoundé II' }
];
...
// Published products matching distributor catalog
const publishedProducts = ref(
  productsStore.products
    .filter(p => p.supplierId === authStore.user?.id || p.supplierId === 'usr_dist_1')
    .map(p => ({
      id: p.id,
      name: p.name,
      category: p.category,
      ...
      storeId: 'store_dist_1_main',
      images: [
        p.image || 'https://images.unsplash.com/photo-1574320590446-2485149ef393?auto=format&fit=crop&w=400&q=80'
      ],
      status: 'Published'
    }))
);
