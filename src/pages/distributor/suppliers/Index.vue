<template>
  <div class="space-y-6">
    ...
    <div class="bg-[var(--color-surface-elevated)] border border-[var(--color-border)] p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
      ...
      <router-link to="/distributor/suppliers/create" class="px-4 py-2 bg-[var(--color-primary)] text-white hover:opacity-90 rounded-lg text-sm font-semibold transition flex items-center space-x-2">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-[var(--color-border)]">
            <tr v-for="sup in paginatedSuppliers" :key="sup.id" class="hover:bg-[var(--color-surface-hover)] transition text-sm">
              <td class="p-4 font-bold text-[var(--color-primary)]">
                <router-link :to="'/distributor/suppliers/' + sup.id" class="hover:underline">{{ sup.id }}</router-link>
              </td>
              <td class="p-4">
              ...
              <td class="p-4 text-right">
                <router-link 
                  :to="'/distributor/suppliers/' + sup.id"
                ...
                <button @click.prevent="itemToDelete = sup; showDeleteModal = true;" class="p-2 ml-2 inline-flex items-center justify-center bg-[var(--color-surface-elevated)] hover:bg-red-500/10 text-[var(--color-text-secondary)] hover:text-red-500 rounded-lg transition border border-[var(--color-border)]">
...
import { b2b_suppliers } from '@/utils/distributor_db';
import { EyeIcon, PlusIcon, MagnifyingGlassIcon, TrashIcon } from '@heroicons/vue/24/outline';
...
function confirmDelete() {
  if (itemToDelete.value) {
    b2b_suppliers.value = b2b_suppliers.value.filter(s => s.id !== itemToDelete.value.id);
