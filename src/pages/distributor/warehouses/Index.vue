<template>
  <div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="bg-[var(--color-surface-elevated)] border border-[var(--color-border)] p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
      ...
      <router-link to="/distributor/warehouses/create" class="px-4 py-2 bg-[var(--color-primary)] text-white hover:opacity-90 rounded-lg text-sm font-semibold transition flex items-center space-x-2">
      ...
      >
        ...
        <div class="p-4 bg-[var(--color-surface-hover)] border-t border-[var(--color-border)] flex justify-between items-center shrink-0">
          ...
          <div class="flex items-center space-x-2">
            <router-link 
              :to="'/distributor/warehouses/' + warehouse.id"
            ...
            <button @click.prevent="itemToDelete = warehouse; showDeleteModal = true;" class="p-1.5 inline-flex items-center justify-center bg-[var(--color-surface-elevated)] hover:bg-red-500/10 text-[var(--color-text-secondary)] hover:text-red-500 rounded-lg transition border border-[var(--color-border)]">
    ...
    <div v-if="paginatedWarehouses.length === 0" class="text-center py-12 text-[var(--color-text-muted)]">
      Aucun entrepôt trouvé.
    </div>

    <div class="p-4" v-if="filteredWarehouses.length > 0">
      <Pagination 
...
import { warehouses } from '@/utils/distributor_db';
import { BuildingOfficeIcon, PlusIcon, MagnifyingGlassIcon, MapPinIcon, TrashIcon } from '@heroicons/vue/24/outline';
...
const itemsPerPage = 6; // cards layout, 6 is good
...
function confirmDelete() {
  if (itemToDelete.value) {
    warehouses.value = warehouses.value.filter(w => w.id !== itemToDelete.value.id);
...
const paginatedWarehouses = computed(() => {
  ...
  return filteredWarehouses.value.slice(start, start + itemsPerPage);
});

watch(filteredWarehouses, () => {
  currentPage.value = 1;
});
</script>
