...
const globalProducts = computed(() => {
  return productsStore.products.filter(p => !verificationProducts.value.some(vp => vp.productId === p.id));
});
...
onMounted(() => {
  if (deliveryNote.value) {
    verificationProducts.value = deliveryNote.value.products.map(p => ({
      ...
      name: p.name || getProductName(p.productId || p.product_id),
...
const getProductName = (id) => {
  const p = productsStore.products.find(x => x.id === id);
...
const addUnexpectedProduct = () => {
  ...
  const prod = productsStore.products.find(p => p.id === selectedUnexpectedProductId.value);
  if (prod) {
  ...
  if (dn) {
    ...
    verificationProducts.value.forEach(item => {
      if (item.checked && item.quantity_received > 0) {
        // Adjust recipient catalog stock
