<template>
  <div class="space-y-6 max-w-5xl mx-auto" id="order-detail-workspace">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[var(--color-border)] pb-4 gap-4 select-none">
      <div class="space-y-1">
        <router-link to="/distributor/purchases" class="text-sm text-[var(--color-primary)] font-mono hover:underline flex items-center space-x-2">
        ...
        <h1 class="text-xl font-bold text-[var(--color-text-primary)] flex items-center space-x-2">
          <span>Détail du Bon de Commande B2B</span>
      ...
      <div class="flex gap-2 shrink-0">
        ...
        <router-link
          ...
          :to="'/distributor/purchases/track/' + purchaseId"
          custom
        >
          <a
            :href="href"
    ...
    >
      ...
      <div class="space-y-0.5 text-[var(--color-text-primary)]">
        <p class="font-bold">Bon de commande envoyé aux partenaires industriels.</p>
        <p class="text-[var(--color-text-secondary)] text-xs">Le dispatch prépare vos conteneurs. Les fonds de séquestre sont bloqués jusqu'à la livraison conforme en entrepôt.</p>
      </div>
    </div>
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white text-slate-900 p-8 shadow-md rounded-2xl border border-slate-200 font-sans tracking-tight relative overflow-hidden" id="printable-po-sheet">
          ...
          <div class="flex justify-between items-start pb-6 border-b-2 border-slate-300">
            <div>
              <div class="text-xl font-black uppercase tracking-wide text-slate-900 font-mono flex items-center space-x-1.5">
                <span>GROUPE DISTRIBUTION</span>
              </div>
              <p class="text-sm text-slate-700 font-mono mt-2 leading-relaxed">
                Yaoundé, Bastos - Cameroun<br>
                Service Logistique Amont B2B<br>
                logistique@groupedistrib.cm
              </p>
            </div>
            <div class="text-right">
              <span class="text-slate-900 font-black font-mono text-sm uppercase bg-slate-100 border border-slate-300 px-3.5 py-1.5 rounded">BON DE COMMANDE GROSSISTE</span>
          ...
          <div class="grid grid-cols-2 gap-6 py-6 border-b-2 border-slate-300 text-sm">
            <div class="space-y-2">
              <span class="block text-xs font-mono text-slate-500 uppercase tracking-wider font-extrabold">Client / Grossiste (Moi)</span>
              <p class="text-base font-extrabold text-slate-900">GROUPE DISTRIBUTION B2B</p>
              <p class="text-slate-700 text-xs leading-relaxed font-medium">
                Yaoundé Central Warehouse<br>
                Mode de Paiement: {{ purchase?.paymentMethod === 'Escrow' ? 'Séquestre Sécurisé' : 'À la livraison' }}
              </p>
            </div>
            
            <div class="space-y-2">
              <span class="block text-xs font-mono text-slate-500 uppercase tracking-wider font-extrabold">Fournisseurs Industriels</span>
              <p class="text-base font-extrabold text-slate-900">{{ purchase?.supplierIds.map(getSupplierName).join(', ') }}</p>
              <p class="text-slate-700 text-xs leading-relaxed font-medium">
                Livraison de gros attendue : {{ formatDate(purchase?.expectedDate) }}
              </p>
            </div>
          </div>
            ...
            <table class="w-full text-left text-sm border-collapse">
              <thead>
                <tr class="border-b-2 border-slate-300 text-xs text-slate-600 font-mono uppercase font-black bg-slate-50">
                  ...
                  <th class="py-3 px-3 border border-slate-300 text-slate-700">Fournisseur</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="py-3.5 px-3 border border-slate-300 text-slate-700 font-mono text-xs">{{ getSupplierName(item.supplierId) }}</td>
                  <td class="py-3.5 px-3 border border-slate-300 text-center font-mono font-black text-slate-900">{{ item.qty }}</td>
                  <td class="py-3.5 px-3 border border-slate-300 text-right font-mono font-bold text-slate-900">{{ formatMoney(item.unitPrice) }}</td>
                  <td class="py-3.5 px-3 border border-slate-300 text-right font-mono font-black text-slate-900">{{ formatMoney(item.qty * item.unitPrice) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 py-6 pt-6 border-t-2 border-slate-300 text-sm">
            <div class="text-xs text-slate-700 font-mono flex flex-col justify-end font-extrabold">
              <p class="text-slate-900 font-black">✓ Bon de commande de gros certifié.</p>
              <p class="mt-1 text-slate-600">Généré via l'interface Distributeur B2B.</p>
            </div>
            ...
            <div class="space-y-2 text-xs font-mono text-slate-700 font-extrabold">
              <div class="flex justify-between border-b border-slate-300 pb-2">
                ...
                <span class="text-slate-900 font-black font-mono">{{ formatMoney(purchase?.amount) }}</span>
              </div>
              <div class="flex justify-between border-b border-slate-300 pb-2">
                <span>TVA (Exonération Fret):</span>
              ...
              <div class="flex justify-between text-slate-900 pt-3 border-t-2 border-slate-400 text-sm font-sans font-black">
                ...
                <span class="font-mono text-sm text-slate-950 font-black">{{ formatMoney(purchase?.amount) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="mt-10 pt-10 border-t-2 border-slate-300 flex justify-between h-20 text-xs font-mono text-slate-700 font-extrabold">
            <div>
              <p class="text-slate-800 font-black">GROUPE DISTRIBUTION (Grossiste)</p>
              <p class="mt-4 italic text-slate-950 font-black">[ Empreinte Numérique ]</p>
            </div>
            <div>
              <p class="text-slate-800 font-black">Les Fournisseurs B2B</p>
              <p v-if="purchase?.status === 'Sent'" class="mt-4 text-amber-600 italic font-black">[ En attente de signature ]</p>
              <p v-else class="mt-4 text-emerald-700 font-black italic">[ Validé &amp; Réceptionné ]</p>
            </div>
          </div>
        </div>
      </div>
...
import { purchases, b2b_suppliers } from '@/utils/distributor_db.js';
...
const timelineEvents = computed(() => {
  ...
  return [
    { label: '1. Bon de Commande Soumis', done: true, desc: 'Requis d\'approvisionnement émis et envoyé aux fournisseurs B2B.' },
