<template>
  <div class="space-y-6">
    ...
    <!-- HEADER BLOCK -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-905 pb-5 shrink-0">
      <div>
        <h1 class="text-xl font-bold font-mono text-slate-100 flex items-center gap-2">
          <ShieldExclamationIcon class="h-5 w-5 text-indigo-400" />
          <span>Product Catalog & Moderation Safety Queue</span>
        </h1>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/45 p-4 rounded-2xl border border-slate-800">
      ...
      <div class="relative w-full md:w-72">
        <input 
          ...
          class="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 rounded-lg p-2.5 pl-8 text-xs text-slate-100 font-mono focus:outline-none transition"
        />
    ...
    <div v-if="filteredProducts.length === 0" class="p-12 text-center bg-slate-900/10 border border-slate-800 rounded-2xl">
      <InboxIcon class="h-10 w-10 text-slate-600 mx-auto" />
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div 
        ...
        :key="prd.id"
        class="bg-[#14141E] border border-slate-800 rounded-2xl overflow-hidden hover:border-slate-700 transition flex flex-col justify-between"
      >
        ...
        <div class="p-5 flex-1 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3 text-xs bg-slate-950 p-3 rounded-xl border border-slate-850">
            ...
            <div class="col-span-2 pt-2 border-t border-slate-900/60 flex items-center justify-between text-[10px] text-slate-400 leading-tight">
        ...
        <div class="px-4 py-3 bg-slate-950 border-t border-slate-850 flex items-center justify-end gap-2 shrink-0">
          <button 
            ...
            class="px-2.5 py-1.5 bg-red-955/20 text-red-400 hover:bg-red-900 hover:text-slate-950 border border-red-950/50 hover:border-red-650 rounded-lg text-[10px] font-mono uppercase font-bold transition flex items-center space-x-1"
          >
            <ShieldExclamationIcon class="w-3.5 h-3.5" />
            <span>Geler / Suspend</span>
          </button>
          <button 
            v-if="prd.moderationStatus !== 'Approved'"
            @click="approveProduct(prd)"
            class="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[10px] font-mono uppercase font-bold transition flex items-center space-x-1"
          >
            <CheckIcon class="w-3.5 h-3.5" />
            <span>Approve Catalog</span>
          </button>
          
          <div v-if="prd.moderationStatus === 'Approved'" class="flex items-center text-[10px] font-mono text-slate-400 space-x-1 py-1">
    ...
    <div class="flex items-center justify-between text-xs font-mono text-slate-500 py-4 border-t border-slate-900 shrink-0">
...
function getBadgeClass(st) {
  if (st === 'Approved') return 'bg-emerald-955 text-emerald-400 border border-emerald-900/50';
  if (st === 'Suspended') return 'bg-red-955 text-red-400 border border-red-900/50';
  return 'bg-amber-955 text-amber-400 border border-amber-900/50 animate-pulse';
}
