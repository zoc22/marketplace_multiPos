<template>
  <div class="min-h-screen bg-[#070b12] text-slate-100 flex flex-col justify-between overflow-x-hidden">
    ...
    <main class="flex-1 py-12 px-4 md:px-8 max-w-5xl mx-auto w-full space-y-8 select-none">
      ...
      <div class="border-b border-[#0e2129] pb-6 mb-8 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          ...
          <h1 class="text-2xl font-extrabold text-white uppercase font-mono mt-2 flex items-center justify-center sm:justify-start space-x-2">
            <UserGroupIcon class="h-6 w-6 text-emerald-450" />
            <span>Coopératives & Fournisseurs Agrées</span>
          </h1>
        ...
        <div class="relative w-full sm:w-64 font-mono text-xs">
          <input 
            ...
            class="w-full bg-[#03060a] border border-[#142630] rounded-xl px-3 py-2 text-xs text-slate-205 focus:outline-none focus:border-emerald-500 placeholder-slate-500 font-mono"
          />
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in-up">
        ...
        <div 
          ...
          :key="supp.id"
          class="bg-[#091116] border border-[#13252f] rounded-2xl p-5.5 space-y-4 hover:border-emerald-900/60 transition duration-300 relative group hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-950/10"
        >
          <!-- Status active badge -->
          <span class="absolute top-4 right-4 bg-[#0d2126] text-emerald-400 text-[8.5px] font-mono px-2 py-0.5 rounded uppercase font-bold border border-emerald-900/45">
            CONFORME Escrow
          </span>

          <!-- Identity and region -->
          <div class="flex items-start space-x-3.5">
            <div class="h-11 w-11 rounded-xl bg-[#0c2020] border border-emerald-950/55 flex items-center justify-center text-emerald-400 shrink-0">
              <BuildingOfficeIcon class="h-6 w-6 text-emerald-400" />
            </div>
            <div class="truncate">
              ...
              <p class="text-[10px] text-cyan-405 font-mono uppercase tracking-wide mt-1">
          ...
          <div class="pt-3 border-t border-slate-900 flex justify-between items-center text-[10px] font-mono">
            <div class="text-slate-550 flex items-center space-x-1">
              <ShieldCheckIcon class="h-4 w-4 text-emerald-500" />
              <span>Capacité Fret: <span class="text-white font-bold uppercase">CEMAC Standard</span></span>
            </div>

            <button 
              @click="routeToSupplierProducts(supp.name)"
              class="px-3.5 py-1.5 bg-emerald-950 text-emerald-450 hover:bg-emerald-400 hover:text-slate-950 font-bold uppercase rounded-lg border border-emerald-900 hover:border-transparent transition"
            >
              Voir ses lots &rarr;
            </button>
          </div>
...
import { UserGroupIcon, BuildingOfficeIcon, ShieldCheckIcon } from '@heroicons/vue/24/outline';
