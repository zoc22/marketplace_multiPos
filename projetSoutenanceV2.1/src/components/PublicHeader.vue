  ...
  <div class="sticky top-0 z-50 select-none">
    ...
    <div class="bg-gradient-to-r from-emerald-950 via-teal-950 to-emerald-900 border-b border-emerald-800 text-emerald-300 py-1.5 px-4 overflow-hidden relative text-[10.5px] font-mono flex items-center shrink-0">
      ...
      <div class="hidden sm:flex items-center space-x-1.5 text-slate-400 text-[10px] shrink-0 ml-4 font-sans">
        ...
        <span class="font-mono text-[9px] text-emerald-400">DEV SIMULATOR ACTIVE</span>
      </div>
    </div>
    ...
    <header class="bg-slate-950/95 backdrop-blur-md border-b border-[#0e181e] px-4 md:px-8 py-3.5 flex items-center justify-between">
      ...
      <router-link to="/" class="flex items-center space-x-2.5 shrink-0 group">
        <div class="h-9 w-9 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-950/40 border border-emerald-400/20 group-hover:scale-105 transition-transform duration-300">
          <svg class="h-5 w-5 text-slate-950" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
          </svg>
        </div>
        <div class="flex flex-col">
          <span class="font-mono text-xs font-bold text-emerald-400 tracking-widest leading-none uppercase">KONG S.A.</span>
      ...
      <nav class="hidden lg:flex items-center space-x-6 text-xs font-mono">
        <router-link 
          to="/products" 
          class="transition text-slate-300 hover:text-emerald-400 px-2 py-1 rounded"
          :class="isRouteActive('/products') ? 'text-emerald-400 font-bold bg-[#0d2226]' : ''"
        >
          Browse Products
        </router-link>
        ...
        <router-link 
          to="/suppliers" 
          class="transition text-slate-300 hover:text-emerald-400 px-2 py-1 rounded"
          :class="isRouteActive('/suppliers') ? 'text-emerald-400 font-bold bg-[#0d2226]' : ''"
        >
          Wholesale Suppliers
        </router-link>
        ...
        <router-link 
          to="/categories" 
          class="transition text-slate-300 hover:text-emerald-400 px-2 py-1 rounded"
          :class="isRouteActive('/categories') ? 'text-emerald-400 font-bold bg-[#0d2226]' : ''"
        >
          Categories Matrix
        </router-link>
        ...
        <router-link 
          to="/about" 
          class="transition text-slate-400 hover:text-emerald-400 px-2 py-1 rounded"
          :class="isRouteActive('/about') ? 'text-emerald-400 font-bold bg-[#0d2226]' : ''"
        >
          Kribi Port Hub Info
        </router-link>
      </nav>
      ...
      <div class="flex items-center space-x-3 text-xs">
        ...
        <button 
          ...
          class="p-2 rounded-xl bg-[#091114] border border-emerald-950/40 text-slate-400 hover:text-emerald-400 transition relative flex items-center"
              ...
              >
                <div class="flex items-center space-x-3.5 truncate">
                  ...
                  <div class="truncate">
                    ...
                    <p class="text-[10px] font-mono text-emerald-400 mt-0.5">{{ formatCFA(prod.price) }} / {{ prod.unit }}</p>
                  </div>
                </div>
                ...
                <div class="flex items-center space-x-1.5 shrink-0">
                  <button 
                    ...
                    class="p-1 px-2.5 bg-emerald-950/60 text-emerald-400 hover:bg-emerald-400 hover:text-slate-950 rounded text-[9px] font-mono font-bold uppercase transition"
                    title="Move to Cart"
                  >
                    Add
                  </button>
        ...
        <div class="relative">
          <router-link 
            to="/cart"
            class="p-2 px-3 rounded-xl bg-emerald-950/30 hover:bg-emerald-950/60 text-emerald-400 border border-emerald-900/40 hover:border-emerald-650 transition relative flex items-center space-x-2 font-mono"
          ...
          >
            ...
            <span class="bg-emerald-500 text-slate-950 text-[10px] font-extrabold px-1.5 py-0.5 rounded-full font-mono shrink-0">
        ...
        <div class="relative">
          <button 
            ...
            class="px-3 py-1.5 bg-gradient-to-r from-[#0d1c22] to-[#122b33] text-emerald-400 hover:text-white rounded-xl border border-emerald-900/50 hover:border-emerald-650 font-mono text-[10.5px] uppercase font-bold tracking-wider flex items-center space-x-1 transition duration-150 relative"
          >
            <span class="inline-block h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse mr-1"></span>
            <span>Gateways</span>
          ...
          <div v-show="isPortalMenuOpen" class="origin-top-right absolute right-0 mt-3 w-64 rounded-xl shadow-2xl bg-slate-900 border border-[#14232c] p-3.5 z-50 text-slate-100">
            ...
            <div class="space-y-1.5">
              ...
              >
                <span class="text-emerald-500 font-mono text-xs font-bold shrink-0">🛒</span>
                <div>
              ...
              <router-link 
                ...
                @click="isPortalMenuOpen = false"
              >
                <span class="text-emerald-400 font-mono text-sm font-bold shrink-0">&Xi;</span>
                <div>
              ...
              <router-link 
                to="/admin/dashboard" 
                class="flex items-center space-x-2.5 p-2 rounded-lg bg-slate-950 hover:bg-amber-950/20 hover:text-amber-500 border border-slate-900/60 hover:border-amber-900 transition text-left"
                @click="isPortalMenuOpen = false"
              >
                <span class="text-amber-500 font-mono text-xs font-bold shrink-0">🔑</span>
                <div>
            ...
            <div class="mt-3 pt-2.5 border-t border-slate-800 text-[9px] text-slate-500 font-mono flex justify-between items-center px-1">
              ...
              <span class="text-emerald-500 font-bold uppercase">{{ authStore.user?.roles?.join(', ') || 'GUEST' }}</span>
            </div>
          </div>
        </div>
    ...
    >
      <div class="max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-4">
          <div>
            <h3 class="text-xs font-mono uppercase tracking-wider text-emerald-400 font-extrabold flex items-center space-x-1">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-4 gap-4 text-xs font-mono">
          ...
          >
            ...
            <div class="py-2.5 border-b border-[#0c1e21] flex justify-between md:block">
              ...
              <span :class="prod.stock < 150 ? 'text-red-400 font-bold' : 'text-emerald-400'">
            ...
            <button 
              @click="addPromoToCart(prod)"
              class="w-full mt-3 bg-emerald-950 hover:bg-emerald-400 hover:text-slate-950 font-bold text-[10px] py-2 rounded-lg border border-emerald-900 uppercase transition tracking-wider flex justify-center items-center"
            >
