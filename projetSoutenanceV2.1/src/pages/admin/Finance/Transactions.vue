<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-900 pb-5 shrink-0">
      <div>
        <h1 class="text-xl font-bold font-mono text-slate-100 flex items-center gap-2">
          Platform Financial Transactions Ledger
        </h1>
        <p class="text-xs text-slate-400">Complete immutable record of escrow collections, supplier bank payouts, and payment fees.</p>
      </div>
      ...
      >
        <span>Export Ledger (CSV) &rarr;</span>
      </button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 font-mono text-xs">
      ...
      <div class="p-4 bg-[#090f23] border border-[#121c3b] rounded-xl text-center">
        <span class="text-slate-505 uppercase tracking-widest block">Collection Inflows</span>
        <span class="text-lg font-bold text-emerald-400 mt-1 block">{{ formatCurrency(inflows) }}</span>
      </div>
      <div class="p-4 bg-[#090f23] border border-[#121c3b] rounded-xl text-center">
        <span class="text-slate-505 uppercase tracking-widest block">Payout Outflows</span>
        <span class="text-lg font-bold text-slate-400 mt-1 block">{{ formatCurrency(outflows) }}</span>
      </div>
      <div class="p-4 bg-[#090f23] border border-[#121c3b] rounded-xl text-center">
    ...
    <div class="flex flex-col sm:flex-row items-center gap-3 bg-slate-900/60 p-4 rounded-xl border border-slate-850">
      <div class="relative flex-1 w-full">
        <input 
          ...
          class="w-full bg-slate-950 border border-slate-850 rounded-lg p-2.5 pl-8 text-xs text-slate-100 placeholder-slate-650 focus:outline-none focus:border-indigo-500 font-mono transition"
        />
        <span class="absolute left-3 top-3.5 text-slate-500">&telrec;</span>
      </div>
    ...
    <div class="bg-[#040817] border border-[#121c3b] rounded-2xl overflow-hidden shadow">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-xs font-sans">
          <thead>
            <tr class="text-[10px] font-mono text-slate-505 uppercase border-b border-[#121c3b] bg-slate-950/80">
          ...
          <tbody class="divide-y divide-[#121c3b]/30">
            <tr 
              ...
              :key="tx.id"
              class="hover:bg-slate-900/40 text-slate-300 transition"
            >
              <td class="p-3 pl-5">
                <div class="font-mono text-[10.5px] text-indigo-400 font-semibold truncate max-w-[120px]">{{ tx.reference }}</div>
                <div class="text-[9px] text-slate-450 font-mono">{{ tx.orderReference || 'BANK_AUTOCLEAR' }}</div>
              </td>
              <td class="p-3 text-slate-450 font-mono">{{ formatDateTime(tx.processedAt) }}</td>
              ...
              <td class="p-3 text-right pr-5 font-mono font-bold" :class="tx.amount > 0 ? 'text-emerald-400' : 'text-slate-350'">
